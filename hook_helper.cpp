...
ID3D12CommandQueue** HookHelper::ppCommandQueue_ = nullptr;
...
HRESULT HookHelper::hkPresent(IDXGISwapChain3 *pSwapChain, UINT SyncInterval, UINT Flags) {
    CallRenderCallback(pSwapChain);
    return oPresent_(pSwapChain, SyncInterval, Flags);
}

HRESULT HookHelper::hkPresent1(IDXGISwapChain3 *pSwapChain, UINT SyncInterval, UINT PresentFlags, const DXGI_PRESENT_PARAMETERS *pPresentParameters) {
    CallRenderCallback(pSwapChain);
    return oPresent1_(pSwapChain, SyncInterval, PresentFlags, pPresentParameters);
}

HRESULT HookHelper::hkResizeBuffers(IDXGISwapChain *pSwapChain, UINT BufferCount, UINT Width, UINT Height, DXGI_FORMAT NewFormat, UINT SwapChainFlags) {
    CallCleanupCallback();
    return oResizeBuffers_(pSwapChain, BufferCount, Width, Height, NewFormat, SwapChainFlags);
}

HRESULT HookHelper::hkSetSourceSize(IDXGISwapChain1 *pSwapChain, UINT Width, UINT Height) {
    CallCleanupCallback();
    return oSetSourceSize_(pSwapChain, Width, Height);
}

HRESULT HookHelper::hkResizeBuffers1(IDXGISwapChain3 *pSwapChain, UINT BufferCount, UINT Width, UINT Height, DXGI_FORMAT NewFormat, UINT SwapChainFlags, const UINT *pCreationNodeMask, IUnknown *const *ppPresentQueue) {
    CallCleanupCallback();
    return oResizeBuffers1_(pSwapChain, BufferCount, Width, Height, NewFormat, SwapChainFlags, pCreationNodeMask, ppPresentQueue);
}

void HookHelper::hkExecuteCommandLists(ID3D12CommandQueue *pCommandQueue, UINT NumCommandLists, ID3D12CommandList *ppCommandLists) {
    UpdateCommandQueue(pCommandQueue);
    return oExecuteCommandLists_(pCommandQueue, NumCommandLists, ppCommandLists);
}

HRESULT HookHelper::hkCreateSwapChain(IDXGIFactory *pFactory, IUnknown *pDevice, DXGI_SWAP_CHAIN_DESC *pDesc, IDXGISwapChain **ppSwapChain) {
    CallCleanupCallback();
    return oCreateSwapChain_(pFactory, pDevice, pDesc, ppSwapChain);
}

HRESULT HookHelper::hkCreateSwapChainForHwnd(IDXGIFactory *pFactory, IUnknown *pDevice, HWND hWnd, const DXGI_SWAP_CHAIN_DESC1 *pDesc, const DXGI_SWAP_CHAIN_FULLSCREEN_DESC *pFullscreenDesc, IDXGIOutput *pRestrictToOutput, IDXGISwapChain1 **ppSwapChain) {
    CallCleanupCallback();
    return oCreateSwapChainForHwndChain_(pFactory, pDevice, hWnd, pDesc, pFullscreenDesc, pRestrictToOutput, ppSwapChain);
}

HRESULT HookHelper::hkCreateSwapChainForCoreWindow(IDXGIFactory *pFactory, IUnknown *pDevice, IUnknown *pWindow, const DXGI_SWAP_CHAIN_DESC1 *pDesc, IDXGIOutput *pRestrictToOutput, IDXGISwapChain1 **ppSwapChain) {
    CallCleanupCallback();
    return oCreateSwapChainForCWindowChain_(pFactory, pDevice, pWindow, pDesc, pRestrictToOutput, ppSwapChain);
}

HRESULT HookHelper::hkCreateSwapChainForComposition(IDXGIFactory *pFactory, IUnknown *pDevice, const DXGI_SWAP_CHAIN_DESC1 *pDesc, IDXGIOutput *pRestrictToOutput, IDXGISwapChain1 **ppSwapChain) {
    CallCleanupCallback();
